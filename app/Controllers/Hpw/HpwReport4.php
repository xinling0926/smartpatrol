...
/**
 * 市场部开门率统计表 (新版 SCB-003)
 *
 * 輸入日期起訖（默認昨天開始算往前五天），查詢巡檢報表(報表編號SCB-003)資料的開門數量
 * 圖表部分以路線為單位加總，數據名稱刪除路線名稱裡的"日常"與"巡檢路線"
 */
class HpwReport4 extends AdminController
{
    ...
    public function report(): string
    {
        ...
        if ($fmd01) {
            ...
            if ($detail) {
                ...
                return $this->error(lang('Hpw/hpw_report4.not_report_data'));
            }
        } else {
            return $this->error(lang('Hpw/hpw_report4.not_report_set_data'));
        }
    }
    ...
    private function calculateSummery(array $data, int $fieldCount): array
    {
        ...
        $output = [];
        $avg = new stdClass();
        $avg->name = lang('Hpw/hpw_report4.average');
                            ...
                            if ($d->src1_1) {
                                ...
                                if ($fmd08) {
                                    $item->route = str_replace(
                                        [lang('Hpw/hpw_report4.str_1'), lang('Hpw/hpw_report4.str_2')],
                                        ['', ''],
                                        $fmd08->fmd0804
                                    );
                                }
